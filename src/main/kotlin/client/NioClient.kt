...
fun main() {
    ...
    try {
        while (true) {
            println("---------------------")
            print("[${localAddress}] Send message to server: ")
            ...
            // 서버로부터 메시지 수신
            val receiveMessageSize: Int = socketChannel.read(byteBufferMessage)
            val receiveMessage: String = String(byteBufferMessage.array()).trim()
            byteBufferMessage.clear()

            println("[${localAddress}] Receive message size from server: $receiveMessageSize")
            println("[${localAddress}] Receive message from server: $receiveMessage")
        }
